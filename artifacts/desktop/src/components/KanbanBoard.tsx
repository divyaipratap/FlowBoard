...
const COLUMNS = [
  { id: IssueStatus.todo, title: "To Do", color: "#6b7280" },
  { id: IssueStatus.in_progress, title: "In Progress", color: "#3b82f6" },
  { id: IssueStatus.in_review, title: "In Review", color: "#eab308" },
  { id: IssueStatus.done, title: "Done", color: "#22c55e" },
];
  ...
  const handleDrop = (e: React.DragEvent, status: IssueStatus) => {
    ...
    if (issue && issue.status !== status) {
      ...
      updateIssue.mutate({ issueId, data: { status } }, {
        onError: () => queryClient.invalidateQueries({ queryKey: getListIssuesQueryKey(projectId) }),
      });
    }
  };
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto space-y-3 pr-2 pb-4">
            {issuesByStatus[col.id].map((issue) => (
              <div key={issue.id} draggable onDragStart={(e) => handleDragStart(e, issue.id)}>
